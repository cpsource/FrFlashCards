...
# Supported by your model: "1024x1024", "1024x1536", "1536x1024", or "auto"
...
def generate_once(model: str, prompt: str, size: str) -> bytes:
    result = client.images.generate(
        model=model,
        prompt=prompt,
        size=size,  # must be one of the supported values
    )
...
def generate_image_b64(french: str, english: str) -> bytes:
    ...
    try:
        ...
        # Fallback: ASCII-only, same supported size
        fb_prompt = build_ascii_fallback_prompt(french, english)
        try:
            print("↪︎ Fallback: ASCII-only prompt, size=1024x1024, no label.")
            return generate_once(MODEL, fb_prompt, "1024x1024")
...
def sanity_test():
    """Generate a test image to verify model access & parameters."""
    test_prompt = "Photorealistic red apple on a plain white background, centered, no text."
    try:
        png = generate_once(MODEL, test_prompt, "1024x1024")
