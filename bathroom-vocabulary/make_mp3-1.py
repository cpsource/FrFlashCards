...
- If the French column begins with l’/l' (elision), we append a gender hint to the spoken text:
...
def tts_to_mp3(text: str, out_base: str, voice: str = "alloy", model: str = "gpt-4o-mini-tts") -> str:
    """Generate MP3 using several stable call patterns. Returns path or raises."""
    client = OpenAI()
...
def begins_with_elision(fr: str) -> bool:
    """Return True if string begins with l’ or l' (any spacing after)."""
    # Normalize leading spaces and accept straight or curly apostrophe.
    s = fr.lstrip()
...
def gender_note(gender: str) -> str | None:
    ...
    # If it's plural feminine (e.g., 'les toilettes') or unknown, skip note.
    return None

def build_tts_text(fr: str, gender: str) -> str:
    """Base sentence + optional gender note when French starts with l’/l'."""
    base = f"Ceci est {fr}."
...
def main():
    ...
    try:
        for en, fr, gender in read_rows(csv_file):
            ...
            print(f"▶ [{total}] {en!r} → base='{slug}.mp3' | FR: {fr} | gender={gender or 'n/a'}")
            if add_note:
