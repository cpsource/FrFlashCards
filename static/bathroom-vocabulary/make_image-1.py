...
"""
make_image.py
Generate a PNG image using OpenAI's image API.
Usage:
    python make_image.py "bar of soap"

This will create: bar_of_soap.png
Prompt style:
    "For the bathroom, show only a bar of soap."
...
def main():
    ...
    prompt = (
        f"For the bathroom, show only a single, clean {name}, "
        f"with no related items, accessories, or background objects. "
        f"The {name} should appear alone, empty if applicable, "
        f"centered and isolated on a plain white background."
    )
    
    #prompt = f"For the bathroom, show only a {name}."
    ...
    try:
        # Generate a clean, high-quality image
